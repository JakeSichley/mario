            ...
            if self.base_loc.x > player.x:
                if abs(self.x - player.x) < 64 and self.x == self.base_loc.x:
                    if player.y < 64:
                        ...
                        miny = max(player.y - 80, 0)
                        maxy = min(player.y + 16, self.settings.scr_height - 64)
                    else:
                ...
                # Check Blooper location relative to base location
                    ...
                    if self.y > min(player.y + player.rect.height, self.base_loc.y):
                        self.base_loc = self.rect
                    else:
                        self.y += 1
                        self.status = 'down'
...
Notes: Takes an additional default parameter 'type': G = Gooma, anything else = Koopa
'''


class FakeBowser(Enemy2):
    def __init__(self, screen, settings, left, bot, type='G'):
        self.screen = screen
        self.screen_rect = screen.get_rect()
        ...
        if type == 'G':
            ...
            self.frames['dead'] = [pygame.transform.flip(load('images/Enemies/166.png'), False, True)]
